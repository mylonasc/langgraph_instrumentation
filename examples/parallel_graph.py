...
# --- 1. Define State ---
# The reducer lets parallel nodes append without overwriting each other.
...
def make_analyst_node(role: str, prompt_style: str):
    ...
    def analyst_node(state: BoardState):
        ...
        response = llm.invoke(
            [
                SystemMessage(
                    content=f"You are {role}. {prompt_style} Keep it brief (2 sentences)."
                ),
...
# Node Definitions
node_optimist = make_analyst_node(
    "The Optimist", "Focus only on potential benefits, upside, and excitement."
)
node_skeptic = make_analyst_node(
    "The Skeptic", "Focus only on risks, downsides, and potential failure points."
)
node_realist = make_analyst_node(
    "The Realist", "Focus on costs, implementation details, and practicality."
)
...
def chairperson_node(state: BoardState):
    ...
    response = llm.invoke(
        [
            SystemMessage(
                content="You are the Chairperson. Synthesize the provided opinions into a "
                "final 1-sentence decision."
            ),
